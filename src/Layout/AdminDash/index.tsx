...
const AdminDash = (): JSX.Element => {
  ...
  const { data }: any = useSwr(
    `${adminUrl}user/kunpexchange${token}`,
    fetcher,
    { ...swrData(state) }
  );
  useEffect(() => {
  ...
  if (state == "success")
    return (
      <>
        <section
          className={`${Style.Section} py-[5px] grid`}
          style={{
            maxHeight: `${height}px`,
            minHeight: `${height}px`,
            height: `${height}px`,
        ...
        >
          ...
          <main
            ...
            className={` grid justify-around ${
              toggle ? undefined : Style.smallShow
            }  `}
          >
            <LargeScreenSidebar toggle={toggle} />
            <main
              id={Style.Outlet}
              className="bg-[#EEEFF4] p-[10px] h-auto min-h-full max-h-full"
            >
              <Outlet />
            </main>
          </main>
...
const SmallScreenSidebar = () => {
  return (
    <div
      className="offcanvas offcanvas-start pt-4 border border-red-400"
      tabIndex={-1}
      id="Smallscreensiderbar"
    ...
    >
      ...
      <div
        className={`${Style.Navbar} w-full h-full overflow-y-scroll s offcanvas-body border-red-600`}
      >
        <Links toggle={true} />
      </div>
    </div>
