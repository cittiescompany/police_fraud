...
const BankRegister = (): JSX.Element => {
    ...
    const makeRequest = async (data: any) => {
        setLoading(true)
        try {
            const res = await axios.post(`${adminUrl}user/${state ? 'create_otp_bank' : 'register'}`, data);
            if (res.data.status) {
                if (!state) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (data) => {
            ...
            const createdata = { isOfficer: false, last_name: data.lastname, first_name: data.firstname, email: data.email, phone: data.phonenumber, password: data.password, directLineEmail: data.email, directLineRank: "hh", directLineName: 'hee' }
        ...
        validationSchema: yup.object({
            ...
            email: yup.string().required("Email addres is required").matches(officialEmailPattern, 'Email must be an official Gmail address"'),
    ...
    return (
        ...
        <section id={Style.section}>
            <main className='relative'>
                <img src='/assets/bankOfficer.jpg' className='min-h-[420px] my-auto absolute top-5' alt='newImage' />
