...
const AdminSignup = (): JSX.Element => {
  ...
  const [details, setDetails] = useState<
    | {
        directLineRank: String | any;
        directLineEmail: String | any;
  ...
  const [ranks] = useGetData(`${adminUrl}user/get_all_ranks`);
  ...
  const handleOk = async () => {
    if (open == 1) {
      if (!details.directLineRank)
        return messageApi.warning("Direct manager rank is required");
      if (!details.directLineName)
    ...
    try {
      setLoading(true);
      const rank_id = ranks.data.find((val: any) => val.name == details.rank);
      const response = await axios.post(
        `${adminUrl}user/${open == 1 ? "create_otp" : "register"}`,
        { ...details, ...newData, rank_id }
      );
      ...
      <main className="z-50">
        ...
        <div className="inputBox mb-0 mt-3 p-0 ">
          <FormControl sx={{}} fullWidth size="small">
            ...
            >
              {ranks?.data?.map((val: any, index: number) => (
        ...
        <Modal
          title={open == 1 ? "DIRECT MANAGER EMAIL ADDRESS" : "OTP"}
        ...
        >
          ...
          <button
            className={`${Style.button} mt-4 mx-auto block`}
            onClick={() => {
              handleOk();
            }}
            disabled={loading}
